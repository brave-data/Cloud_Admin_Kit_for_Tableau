...
**Tableau Cloud Manager** は、管理者が日常のメンテナンスで本当に必要な情報を1画面に集約し、変更の見逃しをなくすためのツールです。
...
├── content_audit.py         # 手動実行コンテンツ監査レポート（古いWB・未ログインユーザーなど）
├── requirements.txt         # Python依存パッケージ
├── .env.example             # 環境変数テンプレート
├── maintenance_report.json  # 隔週スケジュールタスクがシステム健全性情報を自動生成
├── README.md                # 英語版README
